...
export class NewOrderPage implements OnInit {
  ...
  async newAddressModal() {
    const newAddrModal = await this.modalController.create({
      component: NewAddressComponent,
      componentProps: { type: 'user', receiverID: null },
      showBackdrop: false,
      cssClass: 'myClass'
    });
    newAddrModal.onWillDismiss().then(data => {
      // console.log('Modal closedd');
      this.Opacity = 1;
      // console.log(data);
      if (data.role === 'done') {
        console.log('Data After Add', data);
        if (data.data.for === 'receiver') {
          this.getReceiverAddress().then(async () => {
            setTimeout(() => {
              this.shipmentFirst.patchValue({
                receiver_address: parseFloat(data.data.id)
              });
            }, 600);
          });
        } else if (data.data.for === 'sender') {
          this.getUserAddresses().then(async () => {
            setTimeout(() => {
              this.shipmentFirst.patchValue({
                sender_address: parseFloat(data.data.id)
              });
            }, 600);
          });
        }
    ...
    this.Opacity = 0;
    return await newAddrModal.present();
  }
  ...
  async newReceiverAddressModal() {
    if (this.shipmentFirst.controls.receiver_id.valid) {
      ...
      const newRecAddrModal = await this.modalController.create({
        component: NewAddressComponent,
        componentProps: {
          type: 'receiver',
      ...
      newRecAddrModal.onWillDismiss().then(data => {
        // console.log('Modal closedd');
        this.Opacity = 1;
        console.log(data);
        if (data.role === 'done') {
      ...
      this.Opacity = 0;
      return await newRecAddrModal.present();
    } else {
      this.showToast(
  ...
  submitOrder() {
    ...
    let orderData = {
      ...
      items: JSON.stringify(this.packages)
    }
